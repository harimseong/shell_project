...
/*   Updated: 2022/06/11 21:50:26 by hseong           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	redirect_out(t_redirect *redirect)
{
	...
	fd = open(redirect->filename, O_WRONLY | O_CREAT | O_TRUNC, 0446);
	// O_NOCTTY?
...
int	redirect_append(t_redirect *redirect)
{
	...
	fd = open(redirect->filename, O_WRONLY | O_CREAT | O_APPEND, 0446);
	if (fd < 0 || dup2(fd, STDOUT_FILENO) < 0)
		return (1);
	redirect->descriptor = fd;
	redirect->redir_type = REDIR_PROCESSED;
